...
#   
# #
from dotG_enrichment import get_gDataFrame as dotG

g_dataframe, attNumerici, attCategorici, search_grid, _ = dotG()
    ...
    for start, end in zip(xp_indices, xp_indices[1:]):
        ...
        for index, row in sub_df.iterrows():
            ...
            if row['e_v'] == 'v':
                ...
                # Gestisco singolarmente l'aggiunta del parametro Days too late che sarà il target da predirre
                node_attributes['target'] = float(row['target'])
...
class TraceDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        # attr = dictattr(PATH,file="attributi.txt")
        ...
        if 'target' in attNumerici:
            attNumerici.remove('target')
        ...
        for G in listGraph:                                                       #ciclo per scorrere i sottografi
            ...
            print(x)
            """
            adj = nx.to_scipy_sparse_array(G).tocoo()                            #prende la matrice di adiacenza del sottografo
